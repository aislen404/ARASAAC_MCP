---
name: mcp-define-tool-schema
description: Skill mcp/define tool schema para ARASAAC Social MCP Platform. Requiere OpenSpec aprobada.
metadata:
  domain: mcp
...
Ejecutar la capacidad `skill.mcp.define_tool_schema` dentro del flujo OpenSpec del proyecto.

## Entradas esperadas

- Contexto de la change OpenSpec activa.
- Artefactos relevantes (`proposal.md`, `design.md`, `tasks.md`, `spec.md`).
- Datos de dominio necesarios para `mcp/define_tool_schema`.
