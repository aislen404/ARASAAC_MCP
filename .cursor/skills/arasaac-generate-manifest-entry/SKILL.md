---
name: arasaac-generate-manifest-entry
description: Skill arasaac/generate manifest entry para ARASAAC Social MCP Platform. Gates críticos: license, privacy, human_review. Requiere OpenSpec aprobada.
metadata:
  domain: arasaac
...
# skill.arasaac.generate_manifest_entry

## Propósito

Ejecutar la capacidad `skill.arasaac.generate_manifest_entry` dentro del flujo OpenSpec del proyecto.

## Entradas esperadas

- Contexto de la change OpenSpec activa.
- Artefactos relevantes (`proposal.md`, `design.md`, `tasks.md`, `spec.md`).
- Datos de dominio necesarios para `arasaac/generate_manifest_entry`.
